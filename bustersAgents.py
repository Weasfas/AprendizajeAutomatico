...
class BustersAgent:
    ...
    def __init__( self, index = 0, inference = "ExactInference", ghostAgents = None, observeEnable = True, elapseTimeEnable = True, actionFn = None, numTraining=100, epsilon=0.5, alpha=0.5, gamma=1):
...
class rr(ReinforcementAgent):
    ...
    def computePosition(self, state):
        ...
        """
        array = gameState.getLegalPacmanActions()
        distanciasMin=map(lambda a: 9999 if (a==None) else a, gameState.data.ghostDistances)
        index=distancias.index(min(x for x in distancias if x is not None))
        pacx = gameState.getPacmanPosition()[0]
        pacy = gameState.getPacmanPosition()[1]
        ghox=gameState.getGhostPositions()[index][0]
        ...
        if   ( pacy-ghoy < 0 ): move2 = Directions.NORTH
        if   ( pacy-ghoy > 0 ): move2 = Directions.SOUTH
        GDMin=min (distanciasMin)

        if(move1=="STOP" and move2=="NORTH"): return GDmin
        if(move1=="EAST" and move2=="NORTH"): return GDmin*2
        if(move1=="EAST" and move2=="STOP"): return GDmin*3
        if(move1=="EAST" and move2=="SOUTH"): return GDmin*4
        if(move1=="STOP" and move2=="SOUTH"): return GDmin*5
        if(move1=="WEST" and move2=="SOUTH"): return GDmin*6
        if(move1=="WEST" and move2=="STOP"): return GDmin*7
        if(move1=="WEST" and move2=="NORTH"): return GDmin*8
